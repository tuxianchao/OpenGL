...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "cubemaps_environment_mapping", NULL, NULL);
    if (window == NULL)
    {
    ...
    {
        FileSystem::getResPath("/textures/skybox/right.jpg"),
        FileSystem::getResPath("/textures/skybox/left.jpg"),
        FileSystem::getResPath("/textures/skybox/top.jpg"),
        FileSystem::getResPath("/textures/skybox/bottom.jpg"),
        FileSystem::getResPath("/textures/skybox/front.jpg"),
        FileSystem::getResPath("/textures/skybox/back.jpg")
    };
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        shader.setVec3("u_CameraPos", camera.m_position);
        ...
        u_Model = glm::translate(u_Model, glm::vec3(2.0f, -10.0f, 0.0f));
        shader.setMat4("u_Model", u_Model);
        ...
        skyboxShader.use();
        u_View = glm::mat4(glm::mat3(camera.getViewMatrix())); // remove translation from the view matrix
        skyboxShader.setMat4("u_View", u_View);
        skyboxShader.setMat4("u_Projection", u_Projection);
        // skybox cube
        glBindVertexArray(skyboxVAO);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_CUBE_MAP, cubemapTexture);
...
unsigned int loadCubemap(std::vector<std::string> faces)
{
    ...
    for (unsigned int i = 0; i < faces.size(); i++)
    {
        ...
        if (data)
        {
            // #define GL_TEXTURE_CUBE_MAP_POSITIVE_X 0x8515 右
            // #define GL_TEXTURE_CUBE_MAP_NEGATIVE_X 0x8516 左
            // #define GL_TEXTURE_CUBE_MAP_POSITIVE_Y 0x8517 上
            // #define GL_TEXTURE_CUBE_MAP_NEGATIVE_Y 0x8518 下
            // #define GL_TEXTURE_CUBE_MAP_POSITIVE_Z 0x8519 后
            // #define GL_TEXTURE_CUBE_MAP_NEGATIVE_Z 0x851A 前
