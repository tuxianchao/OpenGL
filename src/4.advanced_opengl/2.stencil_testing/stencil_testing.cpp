...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "stencil_testing", NULL, NULL);
    if (window == NULL)
    {
    ...
    glStencilFunc(GL_NOTEQUAL, 1, 0xFF); // 可写入模板
    glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE); // 替换
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        shaderSingleColor.setMat4("u_View", u_View);
        shaderSingleColor.setMat4("u_Projection", u_Projection);
        ...
        // 再关闭深度缓冲写入，用不等的逻辑再画大地点（刚好是边框），这次用单色shader画
        ...
        u_Model = glm::mat4(1.0f);
        u_Model = glm::translate(u_Model, glm::vec3(-1.0f, 0.0f, -1.0f));
        u_Model = glm::scale(u_Model, glm::vec3(scale, scale, scale));
        shaderSingleColor.setMat4("u_Model", u_Model);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        u_Model = glm::mat4(1.0f);
        u_Model = glm::translate(u_Model, glm::vec3(2.0f, 0.0f, 0.0f));
        u_Model = glm::scale(u_Model, glm::vec3(scale, scale, scale));
