...
int main()
{
    ...
    // create glfw window
    // =================
    GLFWwindow *window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "coordinate_systems", nullptr, nullptr);
    if (window == nullptr)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 旋转：绕着X轴旋转
        u_Model = glm::rotate(u_Model, glm::radians(-45.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        // 平移：看起来物体后退
        u_View = glm::translate(u_View, glm::vec3(0.0f, 0.0f, -3.0f));
        // 使用一个简单的透视投影
        float aspectRatio = (float)SCREEN_WIDTH / (float)SCREEN_HEIGHT;
        u_Projection = glm::perspective(glm::radians(45.0f), aspectRatio, 0.1f, 1000.0f);
